...
fn golden_toy_soundness() {
    ...
    let t = exact_soundness(&MultiplicativeSubgroup::new(17, 16).unwrap(), 8).unwrap();
    assert_eq!(t.winning, 17);
    assert!(
        (t.soundness - 1.0).abs() < 1e-12,
        "fully broken below transition"
    );
    let t = exact_soundness(&MultiplicativeSubgroup::new(35521, 16).unwrap(), 8).unwrap();
    assert_eq!(
        t.winning, 3281,
    ...
    let t = exact_soundness(&MultiplicativeSubgroup::new(97, 32).unwrap(), 16).unwrap();
