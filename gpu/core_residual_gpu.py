...
def _koetter(xs, ys, k, m, d, p):
    ...
    # x-degree can reach the constraint count -- the table must too
    binom = _binom_table(len(xs) * m * (m + 1) // 2 + dy + 2, p)
...
def members_through_core(p, points, k, word, t, core):
    ...
    try:
        ...
        return []
    qq = _koetter(fxs, targets, kr, m, d, p)
    out = []
    for g in _roth_ruckenstein(qq, kr, p):
...
def list_paired_mirror(p, points, k, word, t, cores=None):
